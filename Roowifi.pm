...
package Robotics::IRobot::Roowifi;
...
Time::HiRes, YAML::Tiny, POSIX, Math::Trig
...
Initializes the socket, connects to the robot, and initiates the OI Interface.
...
sub init {
    ...
    my $startupMsg = '';
    if ($socket->recv($startupMsg, 255, MSG_DONTWAIT)) {
...
sub loadCalibrationData($$) {
	...
	my $calibData=YAML::Tiny->read($file) if (-e $file);
...
Stops the robot motion and sensor streaming and closes communication socket.
...
sub _indirectSensors($$$) {
	...
	my $now=time();
	$sensorState->{deltaT}=$now-$sensorState->{lastSensorRefresh};
...
=item $robot->initSocket()

Initializes the socket.  Returns true on success, false otherwise.
...
sub readData($$) {
	...
	while($length>0) {
		my ($got,$saw);
        if ($self->{replay}) {
            ($got,$saw) = $self->_readReplayData();
        } else {
            $self->{socket}->recv($saw, 8192, MSG_DONTWAIT);
            $got = length($saw);
        }
